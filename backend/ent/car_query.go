...
func (cq *CarQuery) QueryPredicament() *PredicamentQuery {
...
func (cq *CarQuery) WithPredicament(opts ...func(*PredicamentQuery)) *CarQuery {
	query := &PredicamentQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withPredicament = query
	return cq
}
...
func (cq *CarQuery) sqlAll(ctx context.Context) ([]*Car, error) {
	...
	if query := cq.withPredicament; query != nil {
		...
		query.Where(predicate.Predicament(func(s *sql.Selector) {
			s.Where(sql.InValues(car.PredicamentColumn, fks...))
		}))
