...
func (uc *UserCreate) AddPredicamentIDs(ids ...int) *UserCreate {
	uc.mutation.AddPredicamentIDs(ids...)
	return uc
}

// AddPredicament adds the predicament edges to Predicament.
func (uc *UserCreate) AddPredicament(p ...*Predicament) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddPredicamentIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.PredicamentIDs(); len(nodes) > 0 {
