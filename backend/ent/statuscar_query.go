...
type StatuscarQuery struct {
	...
	// eager-loading edges.
	withPredicament *PredicamentQuery
...
// QueryPredicament chains the current query on the predicament edge.
func (sq *StatuscarQuery) QueryPredicament() *PredicamentQuery {
...
//  WithPredicament tells the query-builder to eager-loads the nodes that are connected to
// the "predicament" edge. The optional arguments used to configure the query builder of the edge.
func (sq *StatuscarQuery) WithPredicament(opts ...func(*PredicamentQuery)) *StatuscarQuery {
	query := &PredicamentQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withPredicament = query
	return sq
}
...
func (sq *StatuscarQuery) sqlAll(ctx context.Context) ([]*Statuscar, error) {
	...
	if query := sq.withPredicament; query != nil {
		...
		query.Where(predicate.Predicament(func(s *sql.Selector) {
			s.Where(sql.InValues(statuscar.PredicamentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Predicament = append(node.Edges.Predicament, n)
		}
	}
