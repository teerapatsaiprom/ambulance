...
func (uu *UserUpdate) AddPredicamentIDs(ids ...int) *UserUpdate {
	uu.mutation.AddPredicamentIDs(ids...)
	return uu
}

// AddPredicament adds the predicament edges to Predicament.
func (uu *UserUpdate) AddPredicament(p ...*Predicament) *UserUpdate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uu.AddPredicamentIDs(ids...)
}
...
func (uu *UserUpdate) RemovePredicamentIDs(ids ...int) *UserUpdate {
	uu.mutation.RemovePredicamentIDs(ids...)
	return uu
}

// RemovePredicament removes predicament edges to Predicament.
func (uu *UserUpdate) RemovePredicament(p ...*Predicament) *UserUpdate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uu.RemovePredicamentIDs(ids...)
}
...
func (uu *UserUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := uu.mutation.RemovedPredicamentIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.PredicamentTable,
			Columns: []string{user.PredicamentColumn},
			Bidi:    false,
	...
	if nodes := uu.mutation.PredicamentIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.PredicamentTable,
...
// AddPredicamentIDs adds the predicament edge to Predicament by ids.
func (uuo *UserUpdateOne) AddPredicamentIDs(ids ...int) *UserUpdateOne {
	uuo.mutation.AddPredicamentIDs(ids...)
	return uuo
}

// AddPredicament adds the predicament edges to Predicament.
func (uuo *UserUpdateOne) AddPredicament(p ...*Predicament) *UserUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uuo.AddPredicamentIDs(ids...)
}
...
// RemovePredicamentIDs removes the predicament edge to Predicament by ids.
func (uuo *UserUpdateOne) RemovePredicamentIDs(ids ...int) *UserUpdateOne {
	uuo.mutation.RemovePredicamentIDs(ids...)
	return uuo
}

// RemovePredicament removes predicament edges to Predicament.
func (uuo *UserUpdateOne) RemovePredicament(p ...*Predicament) *UserUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uuo.RemovePredicamentIDs(ids...)
}
...
func (uuo *UserUpdateOne) sqlSave(ctx context.Context) (u *User, err error) {
	...
	if nodes := uuo.mutation.RemovedPredicamentIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.PredicamentTable,
	...
	if nodes := uuo.mutation.PredicamentIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.PredicamentTable,
