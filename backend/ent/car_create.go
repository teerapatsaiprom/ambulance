...
func (cc *CarCreate) AddPredicamentIDs(ids ...int) *CarCreate {
	cc.mutation.AddPredicamentIDs(ids...)
	return cc
}

// AddPredicament adds the predicament edges to Predicament.
func (cc *CarCreate) AddPredicament(p ...*Predicament) *CarCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return cc.AddPredicamentIDs(ids...)
}
...
func (cc *CarCreate) createSpec() (*Car, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.PredicamentIDs(); len(nodes) > 0 {
